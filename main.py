...
def process_tasks(raw_data):
    ...
    l = sorted(
        ...
        key=lambda x: (
            ...
            # 第三排序：章节号（小优先）
            int(re.findall(r'\d+', x['章节'])[0])
            if re.findall(r'\d+', x['章节'])
            else 999
        )
    )
    return l
        ...
        for task in sorted_data:
            ...
            if enter == "1":
                ...
                for p_dict in point:
                    ...
                    if not tab.ele('x://div[@class="tab-item active"]//span[text()="进行中"]'):
                        tab.ele('xpath://span[text()="我的任务"]').click(by_js=True)
                        tab.wait(1)
                ...
                point = [p for p in points if ((p['特殊状态'] != "免考核") and (p['进度'] != "100%")) or ( "免考核" not in (p['特殊状态'] ) and (p['掌握度'] != "100%"))]
